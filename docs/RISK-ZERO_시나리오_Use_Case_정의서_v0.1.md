...
- **문서 소유:** Pixel-Zero 팀
- **적용 범위:** 후처리 이벤트 · 영상 · 위험도 평가 · 웹/모바일 모니터링 · 알림 · 사용자 분류 · 장치 관리
...
| 센서·게이트웨이 | 센서 원본을 하드웨어에서 분석하고 수치형 지표와 후처리 영상만 `ModuleEvent`로 전달한다. |
...
| 구분 | 현재 상태 |
| --- | --- |
| 하드웨어 데이터 | 수치형 지표·후처리 영상 계약과 DB 저장 구조가 구현되어 있다. 실제 하드웨어 통신은 연결해야 한다. |
| 위험도 계산 | 실제 산식은 비어 있다. 시연 데이터는 14, 46, 68, 88의 고정 점수를 사용한다. |
| 신규 센서 사건 | DB에는 저장되지만 실제 위험도 계산이 없어 `pending`으로 남는다. |
| 웹·모바일 | 시나리오 선택, 최신 사건, 장치 상태, 위험 단계와 대응 미리보기를 표시한다. 모바일은 캘린더·검색·상세 조회도 지원한다. |
| 보호자 피드백 | 사건별 피드백 저장 API가 구현되어 있다. |
| 영상 | 임시 파일 복사, 크기 검증, 앱 저장소 보관, 재생 UI와 500MB 용량 정리가 구현됐다. 실제 하드웨어 파일 입력은 연결 전이다. |
| 알림 | 모바일 기기 내부 단계별 알림, 이벤트 중복 방지, 기본 10분 반복 제한이 구현됐다. 원격 보호자 푸시·문자·전화는 실행하지 않는다. |
| 사용자 분류 | 카테고리, 오탐, 중요 표시, 메모와 검색·필터가 구현됐다. |
| 장치 관리 | 여러 로컬 장치 프로필과 상태·삭제가 구현됐다. 실제 BLE 페어링은 연결 전이다. |
...
```mermaid
flowchart TD
    A["현관 행동 또는 장치 상태 변화"] --> B["하드웨어 분석·영상 후처리"]
    B --> C["수치·영상·시각·중복 여부 확인"]
    C -->|유효| D["수치·영상 경로·이벤트 저장"]
...
| UC-15 | 날짜·시간대별 사건 상세 조회 | 거주자, 보호자 | 공통 | 구현됨 |
| UC-16 | 후처리 영상 저장·재생 | 보호자 | 공통 | 인터페이스·저장 구현 |
| UC-17 | 사건 분류·검색·메모 | 거주자, 보호자 | 공통 | 구현됨 |
| UC-18 | 단계별 기기 알림 확인 | 보호자 | 공통 | 로컬 알림 구현 |
| UC-19 | 여러 장치 등록·상태 확인·해제 | 거주자 | 공통 | 로컬 프로필 구현 |
| UC-20 | 모듈 재연결 후 순번 동기화 | 모듈, 모바일 | 공통 | 메모리 모듈 구현 |
...
| 주 행위자 | 거주자, 보호자 |
| 사전 조건 | 도어락 하드웨어가 조작·실패 횟수를 수치형 지표로 전달한다. |
...
1. 하드웨어 계층이 조작 횟수와 인증 실패를 공통 숫자형 metric으로 변환한다.
2. 하드웨어는 반복 횟수와 시간 간격을 분석해 결과 수치를 전달한다.
...
### UC-14. 새로운 하드웨어 지표 추가

| 항목 | 내용 |
| --- | --- |
| 목적 | 기존 DB와 모니터링 구조를 크게 바꾸지 않고 새로운 후처리 지표를 연결한다. |
| 주 행위자 | HW 담당, SW 담당 |
| 사전 조건 | 하드웨어가 출력할 숫자, 단위, 시각, 품질을 정의한 데이터 사전이 있다. |
| 시작 조건 | 새 지표 또는 새 통신 방식이 프로젝트에 추가된다. |
| 관련 구조 | `ModuleGateway`, `ModuleEvent`, `sensor_readings.metric` |
| 현재 상태 | 확장 가능한 저장 구조와 인터페이스 구현됨 |

**기본 흐름**

1. HW 계층이 제조사별 원본 데이터를 분석한다.
2. 분석 결과를 `metric`, 숫자형 `value`, `unit`, `quality`, `capturedAt` 형식으로 만든다.
3. 실제 통신 계층에서 `ModuleGateway` 구현체를 추가한다.
4. 새 metric은 `sensor_readings`의 새 행으로 저장한다.
5. 위험도 엔진이 해당 metric을 사용하지 않더라도 후처리 사건은 정상적으로 저장한다.
6. 위험요소로 사용할 때 엔진 버전과 데이터 사전에 방향성·정규화·결측 처리를 기록한다.

**대안·예외 흐름**

- 알 수 없는 metric은 저장 허용 여부를 설정으로 관리하고 로그에 남긴다.
- 단위가 다르면 하드웨어 계층에서 공통 단위로 변환하거나 명시적인 변환 규칙을 둔다.
- 새 센서 하나를 추가했다는 이유로 기존 평가 결과를 다시 쓰지 않는다.

### UC-15~20. 모바일 운영 Use Case

| ID | 시작 조건 | 기본 흐름 | 예외·현재 한계 |
| --- | --- | --- | --- |
| UC-15 날짜·시간대별 조회 | 사용자가 `상세 조회` 선택 | 월 이동 → 날짜 선택 → 시간대/위험 단계 그룹 → 이벤트 선택 | 해당 날짜 기록이 없으면 빈 상태 표시 |
| UC-16 영상 저장·재생 | 이벤트에 임시 영상 경로 포함 | 앱 저장소 복사 → 크기 확인 → DB 경로 기록 → 상세에서 재생 | 파일이 없으면 입력 대기, 손상·복사 실패 시 ACK하지 않음 |
| UC-17 분류·검색 | 사용자가 이벤트 상세 확인 | 카테고리·오탐·중요·메모 저장 → 제목·내용·메모 및 카테고리 검색 | 로컬 분류이며 위험도 정답 라벨로 자동 확정하지 않음 |
| UC-18 기기 알림 | watch/warning/critical 사건 수신 | 권한·단계 설정 → 이벤트 중복·10분 제한 확인 → 알림 → 확인 완료 | 앱 종료 중 하드웨어를 깨우는 서비스와 원격 푸시는 미구현 |
| UC-19 장치 관리 | 사용자가 설정에서 장치 추가 | ID·이름·BLE/Wi-Fi 등록 → 배터리·저장공간·동기화 상태 확인 → 해제 시 로컬 데이터 삭제 | 등록은 프로필 생성이며 실제 페어링·소유권 검증은 미구현 |
| UC-20 순번 동기화 | 모듈이 재연결됨 | 마지막 저장 sequence 이후 pull → 파일·DB 저장 → ACK → 다음 배치 | 버퍼가 필요한 순번을 덮어쓴 경우 데이터 유실 상태 표시 필요 |
...
| 모바일 캘린더·상세 | 로컬 조회 | `sensor_events`, `risk_assessments`, `processed_videos` |
| 모바일 분류·검색 | 로컬 저장·조회 | `event_reviews` |
| 기기 알림 | OS 알림 API | `notification_deliveries`, `app_settings` |
| 장치 상태·동기화 | `ModuleGateway` / 로컬 조회 | `devices`, `device_status`, `sync_states` |
...
- [x] 웹·모바일에서 DEMO, API, fallback 데이터 출처를 표시한다.
- [ ] `pending`과 데이터 부족, 장치 고장을 구분해 표시한다.
- [x] 사건 상세·캘린더·검색·사용자 분류 화면을 구현한다.
- [ ] 피드백 라벨의 정책 문서·API 명칭을 통일한다.
- [ ] 실제 후처리 영상 파일로 재생·삭제·손상 흐름을 시험한다.
...
- [x] 모바일 로컬 알림의 단계별 설정과 이벤트 중복·10분 반복 제한을 구현한다.
- [ ] 원격 알림 수신자, 조용한 시간, 실패 재시도를 구현한다.
...
8. **UC-15~17 모바일 조회:** 캘린더에서 날짜를 선택하고 이벤트 상세·분류·검색을 시연.
9. **UC-18~19 설정:** 단계별 알림과 장치 프로필·상태 화면을 확인.
...
| 실제 알림 | 로컬 알림은 구현됨. 원격 푸시·문자·전화의 수신자, 실패 재시도, 조용한 시간 |
| 영상 보존 | 현재 500MB 용량 기준과 정책의 7일 기간 기준을 어떻게 함께 적용할지 |
| 장치 연결 | BLE/Wi-Fi 선택, 페어링, 소유권 확인, 앱 종료 중 백그라운드 수신 |
