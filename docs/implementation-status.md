...
| 입력 테스트 | 브라우저 카메라·마이크 동시 녹화, 랜덤 문구, 로컬 재생·저장 |
...
1. 같은 카메라·마이크로 정상·재생·지연·가림 데이터를 수집한다.
2. AV 싱크 모델 한 개를 `AVSyncModelAdapter`에 연결한다.
3. 데이터셋으로 임계값을 조정하고 FAR, FRR, APCER, BPCER를 기록한다.
4. 하드웨어 출력 JSON을 `ModuleEvent` fixture와 비교하는 계약 테스트를 만든다.
5. 문 모형에서 PASS·만료·재전송·전원 재부팅 시험을 한다.
6. 참여자 동의서, 촬영 안내, 삭제 절차를 팀 검토한다.
