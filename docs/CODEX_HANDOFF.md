...
- 기본 브랜치: `main`
- 프로젝트 단계: 캡스톤 MVP 소스 구현 완료, 실물 하드웨어 통합 미검증

## 한 문장으로 설명

ESP32-CAM이 수집한 현관 영상을 Arty A7-100T로 전달하고, FPGA에서 움직임 중심점과 동선을 계산해 웹·모바일에서 이벤트와 후처리 영상을 확인하는 시스템이다. 음성 도어락 요청의 음성·입모양 일치 검증 MVP도 저장소에 함께 유지한다.
...
| 경로 | 역할 | 상태 |
| --- | --- | --- |
| `firmware/esp32-cam` | 카메라, 웹 스트림, GRAY8 UDP 송신 | 소스 작성, 실제 보드 미검증 |
| `fpga/arty-a7-100t/rtl` | 배경 차분·중심점용 AXI4-Lite RTL | 소스·테스트벤치 작성, Vivado 미실행 |
| `fpga/arty-a7-100t/software` | MicroBlaze lwIP·HTTP·동선 처리 | 소스 작성, Vitis·실물 미검증 |
| `fpga/arty-a7-100t/vivado` | Block Design·합성·bitstream Tcl | 작성 완료, Vivado 미실행 |
...
- ESP32-CAM PlatformIO 실제 빌드·업로드·30분 연속 스트림
- Vivado RTL simulation, synthesis, implementation, timing과 실제 bitstream
- Vitis MicroBlaze 애플리케이션 빌드와 Arty 프로그램
...
cd ..
Copy-Item firmware/esp32-cam/include/config.example.h firmware/esp32-cam/include/config.h
```

`config.h`에는 로컬 Wi-Fi와 Arty IP만 기록하고 커밋하지 않는다. `.env`, 촬영 영상, 참여자 정보와 장치 비밀키도 GitHub에 올리지 않는다.
...
기존 보고 기준은 Edge 41개, FPGA 프로토콜·reference·asset 11개, 웹 렌더·API 11개 테스트 통과다. 새 노트북에서는 의존성이나 도구 버전이 다르므로 반드시 다시 실행해 결과를 갱신한다.
...
1. RTL 테스트 두 개가 PASS인지 확인한다.
2. Block Design의 part가 `xc7a100tcsg324-1`인지 확인한다.
3. synthesis와 implementation이 끝나고 WNS가 0 이상인지 확인한다.
4. `.bit`와 `fpga/arty-a7-100t/build/risk_zero_arty_a7_100t.xsa`가 생성되는지 확인한다.
5. XSA로 Vitis MicroBlaze standalone platform을 만들고 `software/src`를 빌드한다.
6. UART에서 IP, UDP 5005, HTTP 80 준비 로그를 확인한다.
7. ESP32-CAM `config.h`에서 FPGA UDP를 활성화하고 업로드한다.
8. ESP32 `/health`의 `framesSent`와 Arty `/trajectory`의 `completedFrames` 증가를 함께 확인한다.
9. 웹에서 실제 Arty IP를 입력하고 중심점 경로를 확인한다.
...
- 첫 FPGA 구동은 DDR3가 아니라 256KB local BRAM profile을 사용한다.
...
> 이 저장소의 `AGENTS.md`와 `docs/CODEX_HANDOFF.md`를 먼저 전부 읽고, `git status`와 최신 커밋을 확인해줘. 기존 작업을 다시 만들지 말고 구현·PC 테스트·실장치 검증을 구분해서 이어가자. 우선 Vivado가 설치됐는지 확인한 뒤 `fpga/arty-a7-100t/BOARD_BRINGUP.md`의 RTL simulation 단계부터 진행해줘. APK는 내가 명시적으로 요청하기 전에는 빌드하거나 업로드하지 마.
